...
v4 multi-user changes:
  ...
  • AllocationEntry helper (user_name + pct) replaces the old John/Jane named fields.
  • Split models refactored to carry a list of AllocationEntry instead of
    the old cost_pct_john / cost_pct_jane fields.
...
def generate_random_pastel_color() -> str:
    """Generates a random pastel hex colour (#rrggbb)."""
    h = random.random()
    s = random.uniform(0.65, 0.85)
    l = random.uniform(0.68, 0.75)
...
class UserCreate(BaseModel):
    ...
    color:     str = Field(default_factory=generate_random_pastel_color, description="CSS hex colour, e.g. #0ea5e9")
    is_active: bool = True
...
class JobCreate(BaseModel):
    name:         Annotated[str, Field(min_length=1, max_length=256)]
    who:          Annotated[str, Field(min_length=1, max_length=256)]
    amount_cents: Annotated[int, Field(gt=0, description="Rate in whole cents")]
    frequency:    Literal["monthly", "weekly", "biweekly", "annual"] = "monthly"
    start_date:   str = Field(..., description="ISO date YYYY-MM-DD")
    end_date:     Optional[str] = Field(None, description="ISO date YYYY-MM-DD or null")
    notes:        Optional[Annotated[str, Field(max_length=512)]] = None
    is_active:    bool = True
    ...
    @model_validator(mode="after")
    def end_date_after_start_date(self) -> "JobCreate":
...
class JobUpdate(BaseModel):
    name:         Optional[Annotated[str, Field(min_length=1, max_length=256)]] = None
    who:          Optional[Annotated[str, Field(min_length=1, max_length=256)]] = None
    amount_cents: Optional[Annotated[int, Field(gt=0)]] = None
    frequency:    Optional[Literal["monthly", "weekly", "biweekly", "annual"]] = None
...
class JointAccountMonthlyDepositUpdate(BaseModel):
...
class JointAccountDashboardResponse(BaseModel):
    ...
    user_deposits:        list[JointAccountMonthlyDepositRow] = Field(default_factory=list)
