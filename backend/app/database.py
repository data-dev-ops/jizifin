...
async def init_db() -> None:
    ...
    async with aiosqlite.connect(DB_PATH) as conn:
        ...
        # ── Migrate existing tables: add tag_id & is_joint columns if missing ───
        ...
        await ensure_column(conn, "expenses", "is_joint", is_joint_def)
        await ensure_column(conn, "projects", "is_joint", is_joint_def)
        await ensure_column(conn, "projects", "allow_subcategories", "INTEGER NOT NULL DEFAULT 1 CHECK(allow_subcategories IN (0, 1))")
        await ensure_column(conn, "tags", "is_joint", is_joint_def)
        await ensure_column(conn, "income", "is_joint", is_joint_def)
        await ensure_column(conn, "recurring_expenses", "is_joint", is_joint_def)
